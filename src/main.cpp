...
void loop()
{
  ...
  const String data = "{\"id\":1,\"temperature\":" + String(temperature) + ",\"upper_limit\":" + String(UPPER_LIMIT) + ",\"critical_limit\":" + String(CRITICAL_LIMIT) + ",\"color\":[" + String(colors[0]) + "," + String(colors[1]) + "," + String(colors[2]) + "]}";
  if (networkManager.isConnected())
    networkManager.httpPOST(String(API_HOST) + ":" + String(API_PORT) + String(API_ENDPOINT), data);
  delay(2000);
}